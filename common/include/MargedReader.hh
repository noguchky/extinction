            ...
            for (auto&& data : tdcData) {
              if (MrSync::Contains(data.Channel)) {
                ...
                if        (fNextMrSync[board].Tdc) {
                  ...
                  const Int_t step = TMath::Nint((Double_t)(data.Tdc - fNextMrSync[board].Tdc) / (Double_t)(fNextMrSync[board].Tdc - fLastMrSync[board].Tdc));
                  // std::cout << "[debug] step == " << step << std::endl;
                  if (step == 1) {
                    fNext2MrSync[board] = data;
                  } else if (step > 1) {
                    const Long64_t dtdc = (data.Tdc - fNextMrSync[board].Tdc) / step;
                    for (Int_t i = step; i > 1; --i) {
                      fRsvdMrSync[board].push_front(data);
                      data.Tdc -= dtdc;
                    }
                    fNext2MrSync[board] = data;
                  } else {
