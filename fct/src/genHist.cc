...
Int_t main(Int_t argc, Char_t** argv) {
  ...
  do {
    ...
    std::string buff;
    Int_t ch; Double_t resolution;
    while (std::getline(ifile, buff)) {
      std::stringstream line(buff);
      if (line >> ch >> resolution) {
        timePerTdc[ch] = resolution * Extinction::nsec;
      }
    }
  } while (false);
  ...
  do {
    ...
    std::string buff;
    Int_t ch; Double_t interval;
    while (std::getline(ifile, buff)) {
      std::stringstream line(buff);
      if (line >> ch >> interval) {
        mrSyncInterval[ch] = interval;
      }
    }
  } while (false);
